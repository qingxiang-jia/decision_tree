...
from utils import *
import agents, random, operator, math
...
class DecisionTree:
    ...
    def outputDecisionTree(self, path):
        """Output the decision tree as a valid python program."""
        output = ['def predict(example):']
        self.code_gen(0, output)
        with open(path, 'w') as f:
            print >> f, ''.join(output)

    def code_gen(self, indent, output):
        """The actual function that outputs the decision tree as a valid python program."""
        name = self.attr
        x = 0
        output.append('\n')
        for (val, subtree) in self.branches.items():
            output.append(' ' * 4 * (indent + 1))
            output.append(self.if_else(x))
            output.append(' example[')
            output.append(str(name))
            output.append('] == ')
            if isinstance(val, basestring): # check if I need to add ' ' around the val
                output.append('\'' + val + '\'')
            else:
            ...
            if isinstance(subtree, DecisionTree):
                subtree.code_gen(indent + 1, output)
            else:
                output.append('\n')
                output.append(' ' * 4 * (indent + 2))
    ...
    def count_nodes(self):
        cnt = [0]   # a trick to force python to pass cnt by reference
        self.count_nodes_rec(self, cnt)
        return cnt[0]

    def count_nodes_rec(self, tree, cnt):
        for k, subtree in tree.__dict__['branches'].items():
            if isinstance(subtree, DecisionTree):
                cnt[0] += 1
                self.count_nodes_rec(subtree, cnt)
...
class DecisionTreeLearner(Learner):
    ...
    def prune(self, dataset, maxDeviation):
        """The wrapper function that connects the recursive function prune_rec."""
        self.prune_rec(self.dt, dataset.examples, maxDeviation)

    def prune_rec(self, tree, examples, cutoff):
        """The recursive pruning function, the core of this program"""
        if isinstance(tree, DecisionTree):  # if this is a decision, then check if we can prune it
            new_examples = self.split_by(tree.__dict__['attr'], examples)
            dev = 0  # will hold the result of chi-square, declared here for debugging reason
            if self.can_prune(tree):  # check if we can prune this node (i.e., if we already at the bottom)
                dev = self.chi_square(tree, new_examples)
                if dev < cutoff:  # prune, return true so that the caller will handle the pruning
                    return True
            else:  # if we cannot prune this node (not yet reach the bottom), then recursively try to prune its children
                k_ind = 0  # meaning: k_index
                for value, subtree in tree.__dict__['branches'].items():  # iterating its children
                    if self.prune_rec(subtree, new_examples[k_ind][1], cutoff):  # should prune subtree
                        tree.__dict__['branches'][value] = self.majority_value(new_examples[k_ind][1])  # pruning
                    k_ind += 1
                if self.can_prune(tree):  # this is a contronversal part, please refer to my report
                    dev_after = self.chi_square(tree, new_examples)
                    if dev_after - dev == 0:
                        return True
                    # if dev_after < cutoff:
                    #     return True

    def chi_square(self, leaf, examples):
        """Compute chi-square for the fiven node, and corresponding examples"""
        vals = set()  # set of possible final outcomes
        for attr_val, val in leaf.__dict__['branches'].items():
            vals.add(val)
        vals = list(vals) # list of possible final outcomes

        cnt = {}  # this block of code deals with observed counts
        cnt_all = 0.0
        cnt_k_all = {}
        for k in leaf.__dict__['branches']:
            for val in vals:
                cnt[k, val] = self.k_val_count(k, val, examples)
                self.dict_update(val, cnt[k, val], cnt)    # dict_update(key, val, dict), accumulate if already in
                cnt_all += cnt[k, val]
                self.dict_update(k, cnt[val], cnt_k_all)

        exp_cnt = {}  # this block of code deals with expected counts
        for k in leaf.__dict__['branches']:
            for val in vals:
                if cnt_all == 0:
                    cnt_all = self.zero
                exp_cnt[k, val] = cnt_k_all[k] * (cnt[val] / cnt_all)

        dev = 0.0  # actually computing chi-square in this block of code
        for k in leaf.__dict__['branches']:
            for val in vals:
                if exp_cnt[k, val] == 0:
                    exp_cnt[k, val] = self.zero
                dev += (exp_cnt[k, val] - cnt[k, val])**2 / exp_cnt[k, val]

        return dev

    def dict_update(self, key, val, dict):
        """A convenience function used to update dictionary, if key exits, wht value will accumulate"""
        if key not in dict:
            dict[key] = val
        else:
            dict[key] += val

    def k_val_count(self, k, val, examples):
        """Count the number of examples with k and val matched, e.g. k=2, val='shellfish'."""
        cnt = 0
        example_list = []
        for example_lst in examples:
            if example_lst[0] == k:
                example_list = example_lst[1]
                break
    ...
    def can_prune(self, tree):
        """If none of the elements in the dict are DecisionTree, then we can prune."""
        for key, val in tree.__dict__['branches'].items():
    ...
    def decision_tree_learning(self, examples, attrs, default=None):
        if len(examples) == 0:
            ...
            for (v, examples_i) in self.split_by(best, examples):
                subtree = self.decision_tree_learning(examples_i, removeall(best, attrs), self.majority_value(examples))
                tree.add(v, subtree)
            return tree
    ...
    def majority_value(self, examples):
        ...
        return argmax(self.dataset.values[g], lambda v: self.count(g, v, examples))
    ...
    def information_gain(self, attr, examples):
        def I(examples):
            target = self.dataset.target
            return information_content([self.count(target, v, examples) for v in self.dataset.values[target]])
