...
class DecisionTree:
    ...
    def outputDecisionTree(self, path):
        with open(path, 'w') as f:
            print >>f, self.toPythonCode()

    def count_nodes(self):
        return 1+sum([branch.count_nodes() if isinstance(branch, DecisionTree) else 0 \
            for branch in self.branches.values() ])

    def toPythonCode(self, indent=0):
        if (indent == 0):
            codes = '''def predict(example):\n'''
        else:
            codes = ""
        indent += 1

        # generate the code for all branches 
        firstBranch = True
        for (val, subtree) in self.branches.items():
            # append if/elif statement
            if firstBranch:
                codes += '\t'*indent+("if example[%r] == %r:\n" % (self.attr, val))
                firstBranch = False
            else:
                codes += '\t'*indent+("elif example[%r] == %r:\n" % (self.attr, val))
            if isinstance(subtree, DecisionTree):
                # append subtree
                codes += subtree.toPythonCode(indent+1)
            else:
                codes += '\t'*(indent+1)+('return %r\n' % subtree)
...
class DecisionTreeLearner(Learner):
    ...
    def prune(self, dataset, maxDeviation):
        self.dt = self.decision_tree_pruning(self.dt, dataset.examples, maxDeviation)
    ...
    def decision_tree_pruning(self, tree, examples, maxDeviation):
        if not isinstance(tree, DecisionTree) or len(examples) == 0:
            return tree
        for (v, example_i) in self.split_by(tree.attr, examples):
            tree.branches[v] = self.decision_tree_pruning(tree.branches[v], example_i, maxDeviation)
        if self.onlyLeafDescendants(tree):
            dev = self.deviation(tree, tree.attr, examples)
            if dev < maxDeviation:
                return self.majority_value(examples)  
            else:
                return tree
    ...
    def onlyLeafDescendants(self, tree):
        '''Check if all the descendants of a node are leaves'''
        return isinstance(tree, DecisionTree) and \
            all(not isinstance(elem, DecisionTree) for elem in tree.branches.values())    
         
    def deviation(self, tree, best, examples):
        '''Return the deviation of a node'''
        target = self.dataset.target
        # calculate counts of each value
        counts = [self.count(target, v, examples) \
                    for v in self.dataset.values[target]]
        D = 0
        N = float(len(examples))
        target_num = len(self.dataset.values[target])
        # calculate expected count in each subset
        for (v, examples_i) in self.split_by(best, examples):
            sub_counts = [self.count(target, v, examples_i) \
                        for v in self.dataset.values[target]]
            sub_N = float(len(examples_i))
            expect_count = [c*sub_N/N for c in counts]
            D += sum([math.pow(sub_counts[idx]-expect_count[idx],2.0)/expect_count[idx] \
                    for idx in range(target_num) if expect_count[idx]!=0 ])
        return D
